...
//           key={index}
//           className="border-[#7F56D9] ring-ring/50 block size-6 shrink-0 rounded-full border bg-white shadow-sm transition-[color,box-shadow]  focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50"
//         />
//       ))}
//     </SliderPrimitive.Root>
//   );
...
function Slider({
  ...
  const getLabel = (v: number) => {
    if (v >= 1 && v <= 30) return "Short Length";
    if (v >= 31 && v <= 70) return "Medium Length";
    if (v >= 71 && v <= 100) return "Long Length";
    return v;
  };
      ...
      {Array.from({ length: _values.length }, (_, index) => {
        ...
        return (
          <Tooltip key={index}>
            ...
            <TooltipContent
              side="top"
              className="select-none bg-white py-2 px-3 rounded-lg text-gray-700 font-semibold text-xs leading-[18px]"
              style={{
                boxShadow:
